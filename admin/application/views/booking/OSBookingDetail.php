...
<hr>
            <div class="row">
              ...
              <input type="text" id="close_km_form" class="form-control round"
               placeholder="Closing Km" name="close_km">
              </div>
              </div>
  ...
  function AddRow(){
    ...
    $.ajax({
      ...
      success: function(response){
       ...
       //var open_km=$('#open_km').val();
	    if($('#responsex tr').length==1){
		  $('#open_kmx').val(open_km).prop('disabled', true);
		}
	   
	   $('#close_km_form').val(close_km).prop('disabled', true);
      
       var open_km_form=$('#open_kmx').val();
       var close_km_form=$('#close_km_form').val();
       
       var total_km_form = close_km_form - open_km_form;
       alert(total_km_form);
       
       $('#total_km_form').val(total_km_form).prop('disabled', true);
