...
class Login extends CI_Controller{
	...
	public function CheckUserDetail(){  
        ...
        $this->form_validation->set_rules('admin_login', 'Username', 'required');
        $this->form_validation->set_rules('admin_password', 'Password', 'required');
        if($this->form_validation->run())
        {
            $this->load->model('LoginModel');
            if($data=$this->LoginModel->CheckUserDetail($this->input->post('admin_login'),$this->input->post('admin_password'))){
 
               die('<div class="alert alert-success" >Login success please wait while we redirect</div>'."<script>window.location='".base_url()."Plan/NewPlan '</script>");
            }
            else
            {
